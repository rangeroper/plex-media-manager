...
import os from "os"
import type { Buffer } from "buffer" // <--- NEW IMPORT
...
export class PosterStorage {
  ...
  static async updateConfig(updates: Partial<PosterConfig>): Promise<PosterConfig> {
    ...
    updated.stableDiffusion = updates.stableDiffusion !== undefined ? updates.stableDiffusion : current.stableDiffusion

    updated.externalSources = updates.externalSources !== undefined ? updates.externalSources : current.externalSources

    updated.librarySettings = updates.librarySettings !== undefined ? updates.librarySettings : current.librarySettings
  ...
  /**
   * List all generated posters for a specific item
   */
  ...
  > {
    try {
      ...
      // Filter files that match this ratingKey
      const posterFiles = files
        .filter((file) => file.isFile() && file.name.startsWith(`${ratingKey}_`) && file.name.endsWith(".png"))
        .map(async (file) => {
